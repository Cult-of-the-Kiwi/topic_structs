...
async fn build_sc_setup_image() -> String {
    ...
    // Construir imagen usando `docker build`
...
impl FluvioStack {
    pub async fn get_fluvio_handler() -> anyhow::Result<FluvioHandler<DevcordEvent>> {
        ...
        FluvioHandler::new(Some(&format!("127.0.0.1:{}", stack.sc.get_host_port_ipv4(9003).await?)))
    }
    async fn new() -> Self {
        ...
        let name = parts.next().unwrap(); // "sc-setup"
        let tag = parts.next().unwrap_or("latest"); // "latest" si no hay tag
        // SC Setup
...
async fn test_fluvio_stack() {
    ...
    // Puertos asignados aleatoriamente por testcontainers
    let sc_port = stack.sc.get_host_port_ipv4(9003).await.unwrap();
    let spu_port = stack.spu.get_host_port_ipv4(9010).await.unwrap();

    let fluvio = Fluvio::connect_with_config(&FluvioClusterConfig::new(format!("127.0.0.1:{sc_port}"))).await.unwrap();
    let admin = fluvio.admin().await;
    println!("SC running on localhost:{sc_port}");
    println!("SPU running on localhost:{spu_port}");

    // Aquí iría tu lógica de tests contra Fluvio
    // Ejemplo: assert que los contenedores realmente están levantados
    assert!(sc_port > 0);
    assert!(spu_port > 0);

    // Cuando el test termina, todos los contenedores se eliminan automáticamente
}
